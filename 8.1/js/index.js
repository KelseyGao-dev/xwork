$(document).ready(function(){ /* 加载完页面执行 */
    ...
    $(document).scroll(function () {
        ...
        if ($(document).scrollTop()<=200) {
            $('#nav_1').css({"position":"relative","z-index":"99","top":"0"})
        }
    })
});
...
//              _ul.insertBefore(cloneLastLi,ali[0]);
    ...
    //第一个小圆点添加样式
    item.children[0].classList.add('activea');
    //给分页器添加点击事件
    for(let j=0;j<item.children.length;j++){
            item.children[j].addEventListener('click',()=>{
                for(let q=0;q<item.children.length;q++){
                    item.children[q].classList.remove('activea');
                }
                item.children[j].classList.add('activea');
                index = j+1;
